...
const GEMINI_GET_API_KEY_URL = 'https://aistudio.google.com/app/apikey';
...
const TRANSLATIONS = {
  vi: {
    ...
    getApiKeyLink: 'Lấy API key tại Google AI Studio',
    add: 'THÊM',
    remove: 'XÓA',
  ...
  en: {
    ...
    getApiKeyLink: 'Get API key at Google AI Studio',
    add: 'ADD',
    remove: 'REMOVE',
      ...
      <div className="space-y-8 animate-[fadeIn_0.3s_ease-out] pb-20">
          ...
          <div className="space-y-4">
              ...
              <p className="text-[10px] text-slate-500">
                  <a href={GEMINI_GET_API_KEY_URL} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-300 underline">{t.getApiKeyLink}</a>
                  {' → aistudio.google.com'}
              </p>
              <div className="flex gap-2">
                  <input
                      ...
                      onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); const v = newKeyInput.trim(); if (v && !keyPool.includes(v)) { setKeyPool([...keyPool, v]); setNewKeyInput(''); triggerUISound('success'); } } }}
                      placeholder={t.enterApiKey}
                      className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-indigo-500 font-mono"
                  />
                  <button
                      onClick={() => {
                          const v = newKeyInput.trim();
                          if (!v) return;
                          if (keyPool.includes(v)) { triggerUISound('click'); return; }
                          setKeyPool([...keyPool, v]);
                          setNewKeyInput('');
                          triggerUISound('success');
                      }}
                      className="px-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-bold transition-colors whitespace-nowrap"
                  >
                      {t.add}
                  </button>
              </div>
              <p className="text-[9px] text-slate-500">Key được lưu theo tài khoản (localStorage + đồng bộ VPS nếu có).</p>
              <div className="space-y-1">
                  {keyPool.length === 0 ? (
                      <p className="text-[10px] text-slate-500 italic">Chưa có key. Nhập key ở trên rồi bấm {t.add}.</p>
                  ) : (
                      keyPool.map((k, i) => (
                          <div key={i} className="flex justify-between items-center bg-slate-800/50 px-3 py-2 rounded-lg text-[10px] font-mono border border-slate-700/50">
                              ...
                              <button type="button" onClick={() => { setKeyPool(keyPool.filter((_, idx) => idx !== i)); triggerUISound('click'); }} className="text-red-500 hover:text-red-400 font-bold">{t.remove}</button>
                          </div>
                      ))
                  )}
              </div>
